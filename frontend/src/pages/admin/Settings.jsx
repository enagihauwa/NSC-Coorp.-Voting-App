...
const SettingsPage = () => {
  ...
  const isDateKey = (key) => /time|date|end/.test(key || '');
  ...
  const handleEditSetting = (key, value) => {
    ...
    if (isDateKey(key)) {
      ...
      if (!isNaN(d.getTime())) displayValue = d.toISOString().slice(0, 16);
    }
    setEditDialog({ open: true, key, value: displayValue });
  };

  const handleSaveSetting = async () => {
    let value = editDialog.value;
    if (isDateKey(editDialog.key) && value) {
      const d = new Date(value);
      if (!isNaN(d.getTime())) value = d.toISOString();
    }
    await dispatch(updateSetting({ key: editDialog.key, value }));
  ...
  const handleBackup = () => {
    Swal.fire('Not available', 'Database backup is not yet supported by the server.', 'info');
  };

  const handleRestore = () => {
    Swal.fire('Not available', 'Database restore is not yet supported by the server.', 'info');
  };
      ...
      <Box>
            ...
            <Paper elevation={0} sx={{ p: 3, border: '1px solid', borderColor: 'divider', borderRadius: 2, mb: 3 }}>
              ...
              <FormControlLabel
                control={<Switch checked={electionOpen} onChange={handleToggleElection}
                  sx={{ '& .MuiSwitch-thumb': { bgcolor: electionOpen ? 'primary.main' : '#ef4444' },
                ...
                label={<Box><Typography variant="body1" fontWeight={500}>Election Status</Typography>
                  <Chip label={electionOpen ? 'Open' : 'Closed'} size="small"
                    sx={{ mt: 0.5, fontWeight: 600, bgcolor: electionOpen ? 'rgba(22,163,74,0.1)' : 'rgba(239,68,68,0.1)', color: electionOpen ? 'primary.main' : '#ef4444' }} /></Box>}
                    ...
                    {[['Days', countdown.days], ['Hours', countdown.hours], ['Mins', countdown.minutes], ['Secs', countdown.seconds]].map(([label, val], i) => (
                      <Box key={label} display="flex" alignItems="center" gap={2}>
                        <Box textAlign="center">
                          <Typography variant="h4" fontWeight={700} color="primary.main">{String(val).padStart(2, '0')}</Typography>
                          <Typography variant="caption" color="text.secondary">{label}</Typography>
                        </Box>
                        {i < 3 && <Typography variant="h4" fontWeight={700} color="primary.main">:</Typography>}
                      </Box>
                    ))}
                  </Box>
                </Box>
        ...
        <Dialog open={editDialog.open} onClose={() => setEditDialog({ open: false, key: '', value: '' })} maxWidth="sm" fullWidth>
          ...
          <DialogActions>
            ...
            <Button variant="contained" onClick={handleSaveSetting} sx={{ bgcolor: 'primary.main', '&:hover': { bgcolor: 'primary.dark' } }}>Save</Button>
          </DialogActions>
        </Dialog>
        ...
        <Dialog open={endTimeDialog} onClose={() => setEndTimeDialog(false)} maxWidth="xs" fullWidth>
          ...
          <DialogActions>
            ...
            <Button variant="contained" onClick={handleSaveEndTime}
              sx={{ bgcolor: 'primary.main', '&:hover': { bgcolor: 'primary.dark' } }}>Save</Button>
          </DialogActions>
        </Dialog>
      </Box>
    </ErrorBoundary>
